...
args <- c("3col_test5.csv", file.path("C:", "Users", "bishofij", "Proteomics_Pipeline", "go-elite_r_projects"),"test", file.path("GO-Elite_results", "CompleteResults","ORA_pruned"))
...
modulesData <- as.list(read.csv(paste(file.path(args[2],args[1]))))
#modulesData <- as.list(read.csv(paste(args[2],args[1], sep="/"),sep=",", stringsAsFactors=FALSE,header=T)) 
...
for(i in c(1:(length(uniquemodcolors)))){
  ...
  if (file.exists(file.path(args[2],args[3],go_elite_output,"thismod","GOEliteOUTfileTrailer")) == F) { #**note "_Module" needs to be removed from fileTrailer if setting uniquemodcolors not by module color
    if(file.exists(file.path(args[2],args[3],"/GO-Elite_results/CompleteResults/ORA_pruned/","thismod","_2","GOEliteOUTfileTrailer")) == T) {
      tmp=read.csv(file=file.path(args[2],args[3],"/GO-Elite_results/CompleteResults/ORA_pruned/","thismod","_2","GOEliteOUTfileTrailer"),sep="\t")
    } else {
      next
    } 
  } else {
    tmp=read.csv(file=paste(args[2],args[3],"/GO-Elite_results/CompleteResults/ORA_pruned/",thismod,GOEliteOUTfileTrailer,sep=""),sep="\t") #**note "_Module" needs to be removed from fileTrailer if setting uniquemodcolors not by module color
  }
...
args <- c("test_inputs.csv", file.path("Nephele", "folder", "with", "input"),"out_put_folder_name")
args[1]
c(args[1],args[2])
