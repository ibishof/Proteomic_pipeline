...
for(i in c(1:(length(uniquemodcolors)))){
  ...
  if (file.exists(file.path(args[2],args[3],go_elite_output,"thismod","GOEliteOUTfileTrailer")) == F) { #**note "_Module" needs to be removed from fileTrailer if setting uniquemodcolors not by module color
    if(file.exists(file.path(args[2],args[3],go_elite_output,thismod,"_2","GOEliteOUTfileTrailer")) == T) {
      tmp=read.csv(file=file.path(args[2],args[3],go_elite_output,thismod,"_2",GOEliteOUTfileTrailer,sep=""),sep="\t")
    } else {
      next
    } 
  } else {
    tmp=read.csv(file=file.path(args[2],args[3],go_elite_output,thismod,"GOEliteOUTfileTrailer")) #**note "_Module" needs to be removed from fileTrailer if setting uniquemodcolors not by module color
  }
